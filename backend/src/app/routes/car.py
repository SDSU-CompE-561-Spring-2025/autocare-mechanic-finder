...
def add_car(car: CarCreateFROMSCHEMA, db: Session = Depends(get_db)):
    new_car = car_service.create_car(db=db, car=car)
    return new_car

@router.put("/update")  # Creates /update endpoint in the API
def update_car(car_id: int, car: CarUpdateFROMSCHEMA, db: Session = Depends(get_db)):
    updated_car = car_service.update_car(db=db, car_id=car_id, car=car)
    return updated_car

@router.get("/info/{car_id}")  # Creates /info/{car_id} endpoint in the API
