        ...
        return(
            <div className='grid grid-cols-1 xl:grid-cols-2 gap-x-15 gap-y-1 flex-col'>
                {oilnotif.map((x, index) => (
                    <p className='flex justify-between max-w-lg'>{x.year} {x.BrandName} {x.model} {x.trim}<p className='bg-gray-800 rounded-xl ml-3 px-2 text-white text-xs flex items-center'>Oil Change Needed Soon</p></p>))}
                {airnotif.map((x, index) => (
                    <p className='flex justify-between max-w-lg'>{x.year} {x.BrandName} {x.model} {x.trim}<p className='bg-[#738678] rounded-xl ml-3 px-2 text-white text-xs flex items-center'>Air Filter Change Needed Soon</p></p>))}
            </div>)
    }
    else{
